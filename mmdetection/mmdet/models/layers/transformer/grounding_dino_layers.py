...
from .MetaAdapterF import MetaAdapterF
...
class GroundingDinoTransformerDecoderLayer(
    ...
    def __init__(self,
        ...
        #self.cross_attn_text_cfg['batch_first'] = False
    ...
    def forward(self,
        ...
        #     memory_text = memory_text + self.lora_k(memory_text)
        ...
        #     need_weights=True,
        #     average_attn_weights=False,  # 不平均，保留所有query的注意力
        # )
        ...
        # attn_out: [B, num_q, C]  (if batch_first=True)
        # attn_weights: 形状依赖 mmcv 版本和参数，通常可能是:
        #   - if average_attn_weights=True: [B, num_q, L_text]
        #   - if average_attn_weights=False: [B, num_heads, num_q, L_text]
        # 为了统一，我们下面把它处理成 [B, num_q, L_text]

        # 处理 attn_weights 到 [B, num_q, L_text]
        # if attn_weights is not None:
        #     # 若 attn_weights.shape == (B, num_heads, num_q, L) -> mean over heads
        #     if attn_weights.dim() == 4:
        #         # attn_weights: [B, num_heads, num_q, L_text]
        #         attn_by_query = attn_weights.mean(dim=1)  # -> [B, num_q, L_text]
        #     elif attn_weights.dim() == 3:
        #         # attn_weights: [B, num_q, L_text] (already averaged)
        #         attn_by_query = attn_weights
        #     else:
        #         # fallback: try reshape (rare)
        #         attn_by_query = attn_weights
        # else:
        #     attn_by_query = None

        query = self.norms[1](query)
        #self.last_text_attn = attn_weights
        ...
        # # Meta-Adapter 融合视觉上下文（如果存在）
        # if key is not None:
        #     query = self.metaadapterf(query, key)
...
# TODO: 构建Meta-Transfer模块（不用text_feat)
...
class MetaGroundingDinoTransformerEncoder(DeformableDetrTransformerEncoder):
    ...
    def forward(self,
                ...
                text_self_attention_masks: Tensor = None,
                position_ids: Tensor = None,
                img_feats: Tensor = None):

        output = query
        reference_points = self.get_encoder_reference_points(
            spatial_shapes, valid_ratios, device=query.device)

        # main process # memory_text: torch.Size([1, 57, 256]) output：torch.Size([1, 6885, 256])
        for layer_id, layer in enumerate(self.layers):
            output = layer(
                query=output,
                query_pos=(pos_text if pos_text is not None else None),
        ...
        memory_text_by_vis = output.mean(dim=1, keepdim=True)

        text_attention_mask_by_vis =  (memory_text_by_vis.abs().sum(dim=-1) > 0)

        return memory_text_by_vis, text_attention_mask_by_vis
